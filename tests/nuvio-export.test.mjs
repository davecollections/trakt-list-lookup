...
const sitemapText = readFileSync(new URL("../sitemap.xml", import.meta.url), "utf8");
...
assert.match(sitemapText, /<loc>https:\/\/trakt-list-lookup\.pages\.dev\/<\/loc>/);
...
assert.match(indexHtml, /id="about-credits-open"[^>]*aria-haspopup="dialog"[^>]*>About &amp; Credits<\/button>/);
...
assert.match(stylesText, /\.site-footer-minimal \{/);
...
assert.match(nuvioUiJs, /input\.placeholder = "Cover image URL"/);
assert.match(nuvioUiJs, /clearButton\.textContent = "Reset"/);
assert.match(nuvioUiJs, /Reset folder artwork to default/);
assert.match(nuvioUiJs, /actions\.append\(modeGroup, field, clearButton\)/);
assert.match(nuvioUiJs, /body\.append\(details, actions\)/);
